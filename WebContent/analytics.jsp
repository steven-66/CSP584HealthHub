...
<%@page import="bean.Bestseller, com.mongodb.client.MongoCursor, org.bson.Document"%>
      ...
      <ul class="nav">
        <li class="nav-item px-3">
          <a class="nav-link active" href="admin.jsp"
            ><i class="fas fa-list-ul pr-2" style="color: green"></i>Home</a
          >
        </li>
        <li class="nav-item px-3">
                    ...
                    <div class="filters-form-checkbox">
                    	<div>
	                      <input
	                        ...
	                        value="GREAT_THAN"
	                      />
	                      <label for="chk_home_health">Price Greater Than</label>
	                     </div>
	                     <div>
	                     	<input
	                        type="radio"
	                        ...
	                        value="LESS_THAN"
	                      />
                      		<label for="chk_home_health">Price Less Than</label>
                      	</div>
                      
                    </div>
                    <div class="js-form-type-text">
                        <label for="txt_zipcode">Price</label>
                        <input
                          type="text"
                          id="txt_zipcode"
                          name="price"
                    ...
                    <div class="filters-form-checkbox">
                    	<div>
	                      <input
	                        ...
	                        value="GREAT_THAN"
	                      />
	                      <label for="chk_home_health">Rating Greater Than</label>
	                    </div>
	                    <div>
	                       <input
	                        type="radio"
	                        ...
	                        value="LESS_THAN"
	                      />
	                      <label for="chk_home_health">Rating Less Than</label>
	                     </div>
                    </div>
                  </div>
                  <div class="js-form-type-text">
                        <label for="txt_zipcode">Rating</label>
                        <input
                          type="text"
                          id="txt_zipcode"
                          name="rating"
          ...
          <div class="col-9" id="custom-border">
	         <div class="table-responsive" id="table">
				...
				<table class="table table-striped table-sm">
					<%
		         		Map<Integer, MongoCursor<Document>> data = (Map<Integer, MongoCursor<Document>>)request.getAttribute("res"); 
                        if(data == null)return;
		         		if(data.containsKey(1)){
		         			...
		         			while(cursor.hasNext()){
		         				...
		         				out.print("<tr><td>" + document.getString("value") + "</td><td>" + document.getInteger("count") + "<td></tr>");
		         			}
		         			out.print("</tbody>");
		         		}else{
		         			out.print("<thead><tr><th>Product Name</th><th>Price</th><th>Rating</th><th>Review</th><th>User</th><th>City</th><th>Zipcode</th></tr></thead><tbody>");
		         			MongoCursor<Document> cursor = data.get(2);
		         			while(cursor.hasNext()){
		         				Document document = cursor.next();
		         				out.print("<tr><td>" + document.getString("productName") + "</td><td>" + document.getDouble("productPrice") + "</td><td>"+ document.getInteger("rating") + "</td><td>" + document.getString("text")+ "</td><td>" + document.getString("city") + "</td><td>" + document.getString("zipcode")+"</td></tr>");
		         			}
		         			out.print("</tbody>");
		         		}
		         	%>
